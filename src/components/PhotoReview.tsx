...
const SupervisorReview = () => {
  ...
  const [photos, updatePhotos] = useState([
    ...
    {
      id: 2,
      src: 'https://picjumbo.com/wp-content/uploads/sunset-ocean-1080x720.jpg',
    },
    {
      id: 3,
      src: 'https://free-now.com/fileadmin/Markets/Press/General/free-now-logo-rec-blue.png',
    },
  ])
  const addPhoto = () => {
    updatePhotos([
  ...
  const restCallback = (i: number, dir: number) => {
    ...
    gone.add(i)
    console.log(
      `Photo number ${i} is ${dir > 0 ? 'good' : 'bad'}`,
    )
    setTimeout(() => {
      addPhoto()
    }, 200)
  }
  const [items, set] = useSprings(
    photos.length,
    (index) => ({
      xy: [0, 0],
      zIndex: zIndex(index),
  ...
  const bind = useDrag(
    ({ args: [index], down, movement }) => {
      ...
      const trigger =
        Math.abs(movement[0]) > swipeThreshold && !down
      //@ts-expect-error
      set((i) => {
        if (i !== index) return
        return {
          xy: trigger
            ? [(window.innerWidth + 300) * dir, 0]
            : down
            ? movement
            : [0, 0],
          zIndex: zIndex(i),
  ...
  return (
    <>
      {items.map(({ xy, zIndex, o }, i) =>
        gone.has(photos[i].id) ? null : (
          <Wrapper
            key={i}
            style={{
              transform: xy.to(
                (x, y) =>
                  `translate3d(${x}px, ${y}px, 0px) rotate(${
                    x / 50 > 45 ? 45 : x / 50
                  }deg)`,
              ),
              zIndex: zIndex,
